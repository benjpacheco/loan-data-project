...
def get_rds_endpoint(instance_identifier: str) -> str:
    ...
    rds_instances = rds_client.describe_db_instances(DBInstanceIdentifier=instance_identifier)
...
async def load_last_predictions(window_size: int) -> pd.DataFrame:
    query = predictions.select().order_by(predictions.c.created_at.desc()).limit(window_size)
    rows = await database.fetch_all(query)
    return pd.DataFrame(rows)

@task
async def load_current_data(window_size: int) -> pd.DataFrame:
    current_data = await load_last_predictions(window_size)  # Fetch current data from the database
    return current_data
...
def detect_drift(reference_data: pd.DataFrame, current_data: pd.DataFrame) -> bool:
    ...
    drift_detected = data_drift_dashboard.get_by_tab_name("Data Drift").get_metrics()["data_drift"]["value"] > 0.05
...
def run_training_pipeline():
    try:
        ...
        raise Exception(f"Error triggering training: {e}")

@flow
def drift_detection_and_retraining(reference_data: pd.DataFrame, database_window_size: int):
...
def main():
    ...
    drift_detection_and_retraining.run(reference_data=reference_data, database_window_size=3000)
