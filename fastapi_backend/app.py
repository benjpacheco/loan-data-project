...
async def save_to_database(input: dict, output: dict) -> None:
...
def predict_chargedoff(data: LoanData, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(save_to_database, input=data_dict, output=preds)
...
async def monitor_model_performance(window_size: int = 3000) -> FileResponse:
    ...
    numerical_features = current_data.select_dtypes(include=['int64', 'float64']).columns.tolist()
